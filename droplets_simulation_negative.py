...
def generate_ddpcr_image(
    ...
    summary_stats = {
        ...
        '数值': [
            f"{np.mean(diameters_um):.1f}",
            f"{np.std(diameters_um):.1f}",
            f"{np.std(diameters_um)/np.mean(diameters_um):.1%}",
            f"{actual_inhibition_rate:.1%}",
            f"{np.min(dynamic_inhibition_rates):.1%}",
            f"{np.max(dynamic_inhibition_rates):.1%}"
        ]
    }
